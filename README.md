...
3. **Aplicar as migrations + seed**, na ordem:
   ...
   `11111111-1111-1111-1111-111111111111`, um cardápio, e um
   `asaas_wallet_id` mock pra jornada Pix online funcionar sem conta Asaas
   real).
4. **Criar o primeiro usuário**: Authentication → Users → Add user (não
   existe tela de cadastro pro painel — é por convite, ver seção "Lista de
   espera" no `CLAUDE.md`). Copie o UUID do usuário criado.
...
6. **Preencher `.env.local`** (`cp .env.example .env.local`) com a URL e
   as duas chaves do passo 1, e deixar `NEXT_PUBLIC_ASAAS_MOCK=true` (já
   vem assim no `.env.example`) — testa Pix online e "pagar no caixa" sem
   precisar de conta Asaas real.
7. `npm run dev`. Fluxos pra testar:
   - **Cliente (pagar no caixa):** `/11111111-1111-1111-1111-111111111111/menu`
     → monta pedido → "pagar no caixa" → recebe senha e acompanha em
     `/[loja]/pedido/[pedidoId]`.
   - **Cliente (Pix online, modo mock):** mesmo cardápio → "Pix agora" →
     na tela de acompanhamento, botão "Simular pagamento Pix (modo mock)"
     — sem ele, o pedido fica parado em "aguardando confirmação" pra
     sempre, já que não existe pagador de verdade.
...
Pra validar contra o **Asaas real** (sandbox) antes de produção, é preciso
`NEXT_PUBLIC_ASAAS_MOCK=false`/ausente, conta Asaas sandbox, e um túnel
(ngrok ou similar) apontando pra `/api/webhooks/asaas`, já que `localhost`
não é alcançável pelo Asaas — ver `CLAUDE.md`/Backlog. Não é necessário
pra testar as jornadas localmente com o mock.
...
- `lib/asaas/` — interface `PagamentoProvider`; `client.ts` (implementação real via fetch), `mock-client.ts` (sem rede, pra dev local), `provider.ts` (escolhe uma das duas via `NEXT_PUBLIC_ASAAS_MOCK`).
- `lib/lista-espera/` — cadastro de early access da landing (mesmo padrão de caso de uso + interface + repositório Supabase).
